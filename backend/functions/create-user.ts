...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verifica se caller é admin na tabela de perfis (assumindo tabela public.users com coluna role)
    ...
    // Atualiza a tabela de perfis / users com a role se necessário
    try {
      ...
      if (updateError) {
        ...
        return jsonResponse(
          {
            error:
              "Usuário criado no Auth, mas falha ao inserir/atualizar perfil",
            user: {
              id: createdUser.id,
              email: createdUser.email,
            },
      ...
      return jsonResponse(
        {
          error:
            "Usuário criado no Auth, mas ocorreu um erro ao atualizar perfil",
          user: {
            id: createdUser.id,
            email: createdUser.email,
          },
