...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!email || !password || !full_name) {
      return jsonResponse({ error: "Email, password e full_name são obrigatórios" }, 400);
    }
    ...
    if (createError) {
      ...
      return jsonResponse({ error: "Falha ao criar usuário", details: message }, 400);
    }

    const createdUser = createData?.user ?? null;
    if (!createdUser || !createdUser.id) {
      return jsonResponse({ error: "Usuário criado, mas resposta incompleta do Auth"}, 500);
    }
    ...
    return jsonResponse({
      success: true,
      user: { id: createdUser.id, email: createdUser.email, role: newRole},
    }, 200);
  } catch (err) {
